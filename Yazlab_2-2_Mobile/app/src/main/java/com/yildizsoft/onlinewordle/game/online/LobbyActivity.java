...
import com.yildizsoft.onlinewordle.client.*;
...
public class LobbyActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        new Thread(new LobbyRunnable(this)).start();
    }
    ...
    public void CreatePlayerList(String[] playersData)
    {
        ...
        for(int i = 0; i < playersData.length; i += 2)
        {
            if(WordleClient.GetCurrentPlayer() != null)
            {
                if(WordleClient.GetCurrentPlayer().getUsername().equals(playersData[i])) continue;
            }
